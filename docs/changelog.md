...
- **Performance optimizations**
  - Persistent `git cat-file --batch` process for streaming blob reads
  - Batched reads chunked at 200 to avoid pipe buffer deadlocks
  - `snapshot_contents()` replaces N+1 show() calls with list_files + show_many
  - `search()` pushes filtering into git via `--grep`
  - Auto-restart on broken pipe
...
- **Git layer**
  ...
  - Persistent `cat-file --batch` reader with auto-restart
